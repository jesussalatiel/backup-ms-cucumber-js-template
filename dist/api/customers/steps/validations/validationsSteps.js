...
(0, cucumber_1.When)('API response status code should be {int}', function (statusCode) {
    assert_1.default.equal(this.response.statusCode, statusCode, JSON.stringify(this.response, null, 2));
});
(0, cucumber_1.When)('I have the following Joi schema:', async function (schemas) {
    const apiResponse = this.response.body;
    await Promise.all(schemas.hashes().map(async (row) => {
        try {
            const { default: schema } = await Promise.resolve(`${`../../schemas/${row.schemaName}.ts`}`).then(s => __importStar(require(s)));
