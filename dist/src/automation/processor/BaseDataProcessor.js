...
const ConfValueGenerator_1 = __importDefault(require("@automation/processor/ConfValueGenerator"));
class BaseDataProcessor {
    static replaceRandomTextFromFields(worldType, table) {
        ...
        const updatedData = table.raw().map((row) => row.map((cell, colIndex) => {
            ...
            if (regex.test(cell)) {
                return ConfValueGenerator_1.default.getRandomValueBasedOnHeader(header, worldType, cell);
            }
            return cell;
        }));
        const newTable = new cucumber_1.DataTable(updatedData);
        newTable.hashes().forEach((row) => {
            ConfValueGenerator_1.default.setRandomValueBasedOnHeader(row, worldType);
        });
        return newTable;
    }
}
